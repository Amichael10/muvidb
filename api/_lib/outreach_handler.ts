...
import { serviceSupabase } from '../../scripts/lib/credit_consensus_verifier';
...
export async function handleOutreach(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!authOk.valid) return res.status(401).json({ error: 'Unauthorized admin access' });
  } catch (e: any) {
  ...
  if (action === 'generate_batch') {
    try {
      const count = Number(req.body?.count || req.query.count) || 25;
      const result = await generateQueueBatch(count);
      return res.status(200).json({
        success: true,
  ...
  if (action === 'update_message') {
    try {
      ...
      return res.status(200).json({ success: true, person_id });
    } catch (err: any) {
      return res.status(500).json({ error: err.message });
    }
  }
  ...
  if (action === 'cancel_candidate') {
    try {
      ...
      return res.status(200).json({ success: true, person_id, status: 'skipped' });
    } catch (err: any) {
      return res.status(500).json({ error: err.message });
    }
  }

  return res.status(400).json({ error: 'Unknown outreach action' });
}
