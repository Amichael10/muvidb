-- HomiTV: first-party Nollywood streaming platform.
...
as $$
begin
  if p_platform not in ('netflix', 'prime_video', 'kava', 'docuth', 'ebonylife', 'circuits', 'nollistream', 'homitv') then
    return;
  end if;
    ...
    and pnr.film_id not in (
      ...
      where nullif(btrim(f.title), '') is not null
        and (
          ...
          or (
            p_platform = 'homitv'
            and (f.source = 'homitv' or coalesce(f.streaming_links, '{}'::jsonb) ? 'homitv')
          )
        )
        and not exists (
          select 1
      ...
      order by
        case when p_platform in ('circuits', 'nollistream', 'homitv') then f.updated_at else f.created_at end desc
      limit 10
    );
...
select public.refresh_platform_new_releases('homitv');
