-- Claim credit harvest jobs by each linked YouTube film's latest update time.
--
-- The queue used to denormalize films.created_at into job.priority. The harvest
-- workflow now needs each worker run to resume from the latest updated YouTube
-- movie in the database and then move backward, so priority is based on
-- coalesce(films.updated_at, films.created_at) instead.

set statement_timeout = 0;

create index if not exists films_youtube_updated_at_desc_idx
  on public.films ((coalesce(updated_at, created_at)) desc, id)
...
as $priority$
declare
  film_sort_at timestamptz;
begin
  if coalesce(new.status, '') <> 'pending' then
    return new;
  end if;

  select coalesce(f.updated_at, f.created_at)
    into film_sort_at
  from public.films as f
  where f.id = new.film_id
    and f.youtube_watch_url is not null;

  if film_sort_at is not null then
    new.priority := least(floor(extract(epoch from film_sort_at)), 2000000000)::integer;
  end if;
...
create or replace function public.sync_credit_harvest_job_updated_priority()
returns trigger
language plpgsql
security definer
set search_path = public
as $sync$
declare
  film_sort_at timestamptz := coalesce(new.updated_at, new.created_at);
  film_priority integer;
begin
  if new.youtube_watch_url is null or film_sort_at is null then
    return new;
  end if;

  film_priority := least(floor(extract(epoch from film_sort_at)), 2000000000)::integer;
...
create trigger sync_credit_harvest_job_updated_priority
after insert or update of updated_at, created_at, youtube_watch_url
on public.films
for each row
execute function public.sync_credit_harvest_job_updated_priority();

with pending_priorities as (
  select
    job.id,
    least(floor(extract(epoch from coalesce(f.updated_at, f.created_at))), 2000000000)::integer as refreshed_priority
  from public.credit_harvest_jobs as job
  join public.films as f on f.id = job.film_id
  where job.status = 'pending'
    and f.youtube_watch_url is not null
    and coalesce(f.updated_at, f.created_at) is not null
)
update public.credit_harvest_jobs as job
...
as $review$
  with grouped as (
    select
      ...
      max(coalesce(f.updated_at, f.created_at)) as film_updated
  ...
  from counted
  order by
    counted.film_updated desc nulls last,
...
revoke all on function public.sync_credit_harvest_job_updated_priority() from public;

comment on function public.claim_credit_harvest_job(text) is
  'Claims the pending credit harvest job for the latest updated YouTube film first.';

comment on function public.get_credit_candidate_review_films(text, real, integer, integer) is
  'Groups pending credit candidates by film, ordered by the latest updated YouTube film first.';