-- Optimize get_people_with_counts using CTE to avoid calculating heavy correlated counts on thousands of unpaginated rows
...
DECLARE
  ...
  search_terms TEXT[];
BEGIN
  clean_search := TRIM(COALESCE(p_search, ''));
  
  -- Split search into tokens if provided
  IF clean_search <> '' THEN
    search_terms := string_to_array(lower(clean_search), ' ');
  END IF;
  ...
  WITH filtered_candidates AS (
    ...
    WHERE (
        clean_search = '' 
        OR p.name ILIKE '%' || clean_search || '%'
        OR (p.id::text ILIKE clean_search || '%')
        OR (
  ...
  SELECT 
    ...
    fc.is_verified,
    fc.is_spotlight,
    fc.popularity_score::FLOAT,
    fc.known_for_department,
    (SELECT COUNT(*) FROM public.credits c WHERE c.person_id = fc.id)::BIGINT as traditional_credits_count,
    (
      SELECT COALESCE(COUNT(*), 0)
      FROM public.channel_videos cv
      JOIN public.channels ch ON ch.id = cv.channel_id
      WHERE ch.owner_person_id = fc.id
      AND (
        (fc.known_for_department = 'Actor' AND cv.duration_seconds >= 2100) OR
        (fc.known_for_department = 'Skit Maker' AND cv.duration_seconds >= 900) OR
        (fc.known_for_department NOT IN ('Actor', 'Skit Maker') AND cv.duration_seconds >= 900)
      )
    )::BIGINT as youtube_filmography_count,
    (
      (SELECT COUNT(*) FROM public.credits c WHERE c.person_id = fc.id) +
      (
        SELECT COALESCE(COUNT(*), 0)
        FROM public.channel_videos cv
        JOIN public.channels ch ON ch.id = cv.channel_id
...
GRANT EXECUTE ON FUNCTION public.get_people_with_counts(text, text, text, text, boolean, integer, integer, text) TO anon;
