...
async function findTmdbMatch({ person, creditTitles }) {
  if (person.tmdb_id) {
...
async function buildLocalCandidate(target) {
...
export async function checkPeopleEnrichmentSetup() {
  const checks = await Promise.all([
    supabase.from('people_enrichment_queue').select('id').limit(1),
    supabase.from('people_enrichment_research_runs').select('id').limit(1),
    supabase.from('people_enrichment_evidence').select('id').limit(1),
  ]);
  const failed = checks.find((result) => result.error);
  return { ready: !failed?.error, error: failed?.error || null };
}

export async function listPeopleEnrichment(options = {}) {
...
export async function refreshPeopleEnrichment() {
...
export async function reviewPeopleEnrichment(queueId, status, note = null) {
