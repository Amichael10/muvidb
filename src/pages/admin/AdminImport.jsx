...
const CONTENT_TYPES = {
  movies: {
    ...
    mapping: (row) => ({
      title: row.title,
      year: row.year && !isNaN(parseInt(row.year)) ? parseInt(row.year) : null,
      synopsis: row.synopsis || null,
      poster_url: row.poster_url || null,
      backdrop_url: row.backdrop_url || null,
      runtime_minutes: row.runtime && !isNaN(parseInt(row.runtime)) ? parseInt(row.runtime) : null,
      tmdb_rating: row.rating && !isNaN(parseFloat(row.rating)) ? parseFloat(row.rating) : null,
      tmdb_id: row.tmdb_id && !isNaN(parseInt(row.tmdb_id)) ? parseInt(row.tmdb_id) : null,
      trailer_youtube_id: row.trailer_id || null,
      release_type: row.release_type || (row.youtube_link || row.youtube_watch_url ? 'youtube' : 'cinema'),
