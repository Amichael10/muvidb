    ...
    <div className="min-h-screen bg-bg-dark text-text-primary pb-20">
      ...
      <section className="relative overflow-hidden border-b border-border-dark bg-gradient-to-b from-bg-darker via-bg-dark to-bg-dark py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-yellow/10 border border-accent-yellow/30 text-accent-yellow text-xs font-semibold uppercase tracking-wider mb-4">
          ...
          <div className="mt-8 max-w-md mx-auto relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-surface-dark border border-border-dark rounded-xl text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-accent-yellow transition-colors shadow-lg"
            />
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
        {loading ? (
          ...
          <div className="text-center py-20 bg-surface-dark/50 rounded-2xl border border-border-dark">
            ...
            <h3 className="text-xl font-bold text-white mb-1">No critics found</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((critic) => (
              <Link
                ...
                className="group relative bg-surface-dark/60 border border-border-dark hover:border-accent-yellow/50 rounded-2xl p-6 transition-all duration-300 hover:shadow-xl hover:shadow-accent-yellow/5 hover:-translate-y-1 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="relative">
                      <img
                        ...
                        alt={critic.name}
                        className="w-16 h-16 rounded-full object-cover border-2 border-border-dark group-hover:border-accent-yellow transition-colors shadow-md"
                      />
                      {critic.is_verified && (
                        <div className="absolute -bottom-1 -right-1 bg-accent-yellow text-bg-dark rounded-full p-1 shadow-md" title="Verified Film Critic">
                          <Icon icon="solar:verified-check-bold" className="w-3.5 h-3.5" />
                        </div>
                      )}
                    </div>

                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-surface-light/30 border border-border-dark text-xs text-text-muted font-medium">
                      <Icon icon="solar:notes-bold" className="w-3.5 h-3.5 text-accent-yellow" />
                      {critic.review_count} {critic.review_count === 1 ? 'Review' : 'Reviews'}
                    </span>
                  </div>

                  <h2 className="text-xl font-bold text-white group-hover:text-accent-yellow transition-colors">
                    {critic.name}
                  </h2>
                  <p className="text-xs font-semibold text-accent-yellow/90 mt-0.5">
                ...
                <div className="mt-6 pt-4 border-t border-border-dark/60 flex items-center justify-between text-xs text-text-muted group-hover:text-text-primary transition-colors">
                  <span className="font-mono text-accent-yellow">{critic.handle || '@critic'}</span>
                  <span className="flex items-center gap-1 font-semibold text-accent-yellow group-hover:translate-x-1 transition-transform">
                    View Profile
