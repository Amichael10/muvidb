...
-- that correctly handle unique constraint conflicts in the credits table.
...
BEGIN
    ...
    -- A duplicate conflict occurs if both profiles have the same (film_id, role).
    ...
    -- We use COALESCE to fill in null fields on the primary with data from the secondary.
...
BEGIN
    ...
    -- Delete credits on the secondary film that would cause duplicate conflicts on the primary film.
    -- A duplicate conflict occurs if the same person has the same role on both films.
    DELETE FROM public.credits c_sec
