...
async function findOrCreateFilm(item: typeof reviewMappings[0], articleData: any) {
  const titlesToTry = [item.film_search, item.film_alt_search].filter(Boolean) as string[];
  
  for (const t of titlesToTry) {
    // 1. Exact match
    const { data: exact } = await supabase.from('films').select('id, title, slug, poster_url, year').eq('title', t).maybeSingle();
    if (exact) return exact;

    // 2. ILIKE match
    const { data: ilike } = await supabase.from('films').select('id, title, slug, poster_url, year').ilike('title', t).limit(1);
    if (ilike && ilike.length > 0) return ilike[0];

    // 3. Slug match
    const slug = t.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    const { data: bySlug } = await supabase.from('films').select('id, title, slug, poster_url, year').eq('slug', slug).maybeSingle();
    if (bySlug) return bySlug;
  }

  // If not found, create rich record
  const slug = item.film_search.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  const posterUrl = articleData?.image_url || null;
  const synopsis = articleData?.description || `Official Nollywood production '${item.film_search}' released in ${item.fallback_year}.`;

  const { data: created, error } = await supabase.from('films').insert({
    title: item.film_search,
    slug,
    year: item.fallback_year,
    synopsis,
  ...
  if (error) {
    console.error(`Error creating film ${item.film_search}:`, error);
    return null;
  }
  console.log(`✨ Created full film record in DB: '${created.title}' (${created.id})`);
  return created;
}

async function run() {
  console.log("=== UPSERTING DAMI DAWSON CRITIC PROFILE ===");
  ...
  if (criticErr) {
    console.error("Critic upsert error:", criticErr);
  } else {
    console.log("✅ Dami Dawson critic profile confirmed active & verified.");
  }

  console.log("\n=== INGESTING FILM REVIEWS ===");
  const results = [];
  ...
  for (const m of reviewMappings) {
    ...
    const film = await findOrCreateFilm(m, article);
    if (!film) {
      console.warn(`⚠️ Skipped ${m.film_search} - could not resolve or create.`);
      continue;
    }

    // Check existing review
    const { data: existing } = await supabase.from('critic_reviews')
      .select('id')
      .eq('film_id', film.id)
    ...
    let status = '';
    if (existing) {
      const { error: upErr } = await supabase.from('critic_reviews').update(reviewPayload).eq('id', existing.id);
      if (upErr) {
        console.error(`Error updating review for ${film.title}:`, upErr);
      } else {
      ...
      if (insErr) {
        console.error(`Error inserting review for ${film.title}:`, insErr);
      } else {
        console.log(`🌟 Inserted new review: '${film.title}' (${m.rating}★)`);
        status = 'Inserted';
      }
    }
  ...
  console.log(`\n🎉 Ingestion complete: ${results.length} reviews processed!`);
