...
async function streamAllDatabasePeople() {
  console.log('🚀 STREAMING ALL 10,000+ DATABASE RECORDS FROM SUPABASE...');
  let totalPeople = [];
  let offset = 0;
  const limit = 1000;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log(`  Fetched ${totalPeople.length} people records so far...`);
        offset += limit;
...
async function run10kScan() {
  const people = await streamAllDatabasePeople();
