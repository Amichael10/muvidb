...
const FEMI_PERSON_ID = 'b9d0f1c5-8052-4829-8e79-dfe9facb371e';
const FEMI_NM_ID = 'nm2132401';
...
const ALIAS_MAP = {
  'femi ogedengbe': 'Femi Ogedengbe',
  'femi ogedegbe': 'Femi Ogedengbe',
  'olufemi ogedegbe': 'Femi Ogedengbe',
  'femi ogedemgbe': 'Femi Ogedengbe',
  ...
  'ray eyiwumi': 'Ray Eyiwumi',
  'eyiyemi afolayan': 'Eyiyemi Afolayan',
  'jimoh aliu': 'Jimoh Aliu',
...
function isFemi(name, nmId) {
  if (nmId === FEMI_NM_ID) return true;
  const norm = normalizeName(name).toLowerCase();
  return norm === 'femi ogedengbe';
}
...
function extractPoster(credData) {
  ...
  if (ogImage && ogImage.includes('/images/M/') && !ogImage.includes('imdb_logo') && !ogImage.includes('nopicture')) {
    return ogImage;
  }
  return null;
}
...
function parseCreditsMarkdown(md) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (catMatch) {
      ...
      if (['Director', 'Directors', 'Writer', 'Writers', 'Cast', 'Producer', 'Producers', 'Cinematographer', 'Cinematographers', 'Composer', 'Composers', 'Editor', 'Editors', 'Production Designer', 'Production Management'].some(c => cat.toLowerCase().startsWith(c.toLowerCase()))) {
        currentCategory = cat;
        continue;
      }
    }
...
async function preloadExistingPeople() {
  ...
  // Ensure Femi Ogedengbe is mapped
  personCache.set('femi ogedengbe', FEMI_PERSON_ID);
  personCache.set('femi-ogedengbe-589', FEMI_PERSON_ID);
  personCache.set('femi ogedegbe', FEMI_PERSON_ID);
  personCache.set('olufemi ogedegbe', FEMI_PERSON_ID);
  personCache.set('femi ogedemgbe', FEMI_PERSON_ID);
...
async function bulkResolvePeople(names) {
  ...
  for (const name of names) {
    ...
    if (key === 'femi ogedengbe' || key === 'femi ogedegbe' || key === 'olufemi ogedegbe' || key === 'femi ogedemgbe') {
      resolved.set(name, FEMI_PERSON_ID);
      continue;
    }
...
async function syncAllFemiTitles() {
  await preloadExistingPeople();

  const femiTitles = JSON.parse(fs.readFileSync('scratch/all_femi_titles.json'));
  console.log(`🎬 Syncing ${femiTitles.length} Femi Ogedengbe titles with complete filmography & ensembles...`);

  let filmsCreated = 0;
  let filmsUpdated = 0;
  let creditsAddedTotal = 0;
  const touchedPersonIds = new Set([FEMI_PERSON_ID]);

  for (let i = 0; i < femiTitles.length; i++) {
    const t = femiTitles[i];
    const tt = t.imdbId;
    console.log(`\n[${i + 1}/${femiTitles.length}] Processing ${tt}: "${t.title}" (${t.year || 'N/A'})...`);
    ...
    if (existingByImdb) {
      ...
      const updates = {
        is_nollywood: true,
        poster_url: genuinePoster // Set to genuine poster or null if none
      };
    ...
    // 2. Ensure Femi Ogedengbe's credits are present
    const femiInEnsemble = ensemble.filter(e => isFemi(e.name, e.nmId));
    if (femiInEnsemble.length === 0) {
      ensemble.unshift({
        nmId: FEMI_NM_ID,
        name: 'Femi Ogedengbe',
        role: t.role || 'actor',
      ...
      for (const fe of femiInEnsemble) {
        fe.name = 'Femi Ogedengbe';
        if (!fe.characterName && t.character && fe.role === 'actor') {
          fe.characterName = t.character;
        }
      }
    }
    ...
    for (let b = 0; b < ensemble.length; b++) {
      ...
      const pId = isFemi(mem.name, mem.nmId) ? FEMI_PERSON_ID : resolvedPeople.get(mem.name);
      if (!pId) continue;
  ...
  console.log(`\n======================================================`);
  console.log(`🎉 Films & Credits Sync Complete!`);
  ...
  // 7. Verify Femi Ogedengbe
  const { data: femiProfile } = await supabase
    .from('people')
    .select('id, name, slug, film_count')
    .eq('id', FEMI_PERSON_ID)
    .single();

  const { count: femiCreditCount } = await supabase
    .from('credits')
    .select('*', { count: 'exact', head: true })
    .eq('person_id', FEMI_PERSON_ID);

  console.log('\n🌟 Femi Ogedengbe Final Verification:');
  console.log(`  Name: ${femiProfile?.name}`);
  console.log(`  Slug: ${femiProfile?.slug}`);
  console.log(`  film_count in DB: ${femiProfile?.film_count}`);
  console.log(`  Exact credits count in DB: ${femiCreditCount}`);
}

syncAllFemiTitles().catch(console.error);
