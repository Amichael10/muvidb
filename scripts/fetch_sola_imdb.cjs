...
async function run() {
  // 1. Name page
  const nameData = await scrape('https://www.imdb.com/name/nm2535409/');
  if (nameData?.markdown) {
    fs.writeFileSync('scratch/sola_name_page.md', nameData.markdown);
    console.log('Saved scratch/sola_name_page.md');
  }

  // 2. Search titles by role
  const searchData = await scrape('https://www.imdb.com/search/title/?role=nm2535409');
  if (searchData?.markdown) {
    fs.writeFileSync('scratch/sola_search_results.md', searchData.markdown);
    console.log('Saved scratch/sola_search_results.md');
  }
}
