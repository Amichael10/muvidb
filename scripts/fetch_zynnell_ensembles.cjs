...
const titles = JSON.parse(fs.readFileSync('scratch/zynnell_clean_titles.json'));
...
async function main() {
  console.log(`🎬 Fetching fullcredits for ${titles.length} Zynnell Zuh titles...`);
  ...
  async function worker(workerId) {
    while (nextIdx < pending.length) {
      ...
      console.log(`[Worker ${workerId}] (${item.index}/${titles.length}) Fetching ${item.imdbId} - "${item.cleanTitle}"...`);
      const data = await scrapeWithFirecrawl(url);
      if (data) {
        fs.writeFileSync(`scratch/credits_${item.imdbId}.json`, JSON.stringify(data, null, 2));
        fetchedCount++;
        console.log(`  ✅ [Worker ${workerId}] Saved ${item.imdbId} ("${item.cleanTitle}")`);
      } else {
        failedCount++;
        console.error(`  ❌ [Worker ${workerId}] Failed ${item.imdbId} ("${item.cleanTitle}")`);
      }
