...
const CHARACTER_ROLE_PREFIXES = /^(?:BROTHER|BRO|SISTER|SIS|UNCLE|AUNTY|AUNT|MAMA|BABA|PAPA|PAPPY|PASTOR|ALFA|IMAM|ALHAJI|ALHAJA|DOCTOR|DOC|NURSE|LAWYER|BARRISTER|OFFICER|POLICE|INSPECTOR|SERGEANT|DPO|GATEMAN|GATE\s+MAN|DRIVER|LANDLORD|LANDLADY|MAID|CHAIRMAN|MADAM|ELDER|OMO|ELEGBON|OLORI|BOARD|MEMBERS|MEMBER|GUESTS?|EXTRAS?|DANCERS?|PROPS?|ARTIST|ARTSIST|CREW|CAST|TATTOO|GHOST|BIG\s+FISH)\b/i;
...
function personTextLooksValid(text: string, allowSingleWord = false): boolean {
  ...
  if (/\b(?:agency|ventures|enterprises|properties|limited|ltd|holdings|services|company|consult|logistics|foundation|studio|studios|production|productions|entertainment|props?|costumes|glamour|media(?:\s*pro)?|visuals?|concepts?|pictures|channel|network|tv|board\s*members?|ity\s*guests?|city\s*guests?|props?\s*sets?|set\s*props?)\b/i.test(norm)) {
    return false;
  }
...
const CHARACTER_GROUP_KEYWORDS = /\b(?:FRIENDS?|BOYFRIEND|GIRLFRIEND|HUSBAND|WIFE|WIVES|FAMILY|BROTHERS?|SISTERS?|ELDERS?|GUARDS?|MAIDS?|VILLAGERS?|STUDENTS?|CHILDREN|GUESTS?|WORKERS?|OFFICERS?|MEMBERS?|BOARD|BOARD\s+MEMBERS?|CUSTOMERS?|GIRLS?|BOYS?|WOMEN|MEN|EXTRAS?|DANCERS?|KIDNAPPERS?|THUGS?|NURSES?|DOCTORS?|PATIENTS?|COUPLE|CROWD)\b/i;
