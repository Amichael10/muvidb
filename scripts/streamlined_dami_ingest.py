...
def search_supabase_film(title):
    try:
        ...
        # Contains pattern
        res_cont = session.get(f"{supabase_url}/rest/v1/films", params={'title': f'ilike.%{title}%', 'select': 'id,title,slug,poster_url,year', 'limit': '1'}, headers=db_headers, timeout=5)
...
def create_film_in_supabase(title, fallback_year, fallback_poster, fallback_desc, genres):
    slug = re.sub(r'[^a-z0-9]+', '-', title.lower()).strip('-')
    ...
    payload = {
        ...
        'slug': slug,
        'year': fallback_year,
        'synopsis': fallback_desc or f"Official Nollywood production '{title}' released in {fallback_year}.",
    ...
    try:
        ...
        if res.status_code in [200, 201]:
            created = res.json()
            print(f"  ✨ Created full film record in DB: '{title}' (ID: {created[0]['id']})")
            return created[0]
        else:
...
print("\n========================================================")
print("🚀 RUNNING STREAMLINED DAMI DAWSON REVIEWS INGESTION")
print("========================================================\n")
...
for m in review_mappings:
    ...
    if film:
        ...
        print(f"   ⚡ Film not in DB. Creating hydrated film record from article data...")
...
out_summary = r"C:\Users\User\.gemini\antigravity\brain\3608c7a3-6ac6-40c8-9e37-f3bc44e75f57\scratch\dami_dawson_final_summary.json"
